...
from message_bus import send_message, get_messages, get_full_log
...
def main():
    ...
    if spec_data:
        ...
        if verdict["verdict"] == "fail":
            print("\n⚠️ CEO requesting revision from Product Agent!")
        ...
        print("  ⚠️ CEO could not find spec to review.")
        spec_data = spec

    # ── STEP 4: Engineer + Marketing run in parallel (simulated) ──
    pr_url, issue_url = run_engineer()
    pr_url = pr_url if pr_url else "https://github.com"
    ...
    if qa_result and qa_result["review"]["verdict"] == "fail":
        print("\n⚠️ QA FAILED — CEO requesting Engineer revision!")
        send_message("ceo", "engineer", "revision_request", {
            "issues": qa_result["review"].get("html_issues", []),
    ...
    try:
        qa_verdict = qa_result["review"]["verdict"] if qa_result else "N/A"
        summary = {
            "channel": "#launches",
            "blocks": [
                {
        ...
        if res.json().get("ok"):
            ...
            print(f"⚠️ Slack error: {res.json().get('error')}")
